...
const { AUDIO_SOUND_RATE, AUDIO_CODEC_NAME, VIDEO_CODEC_NAME } = AV;
...
class RTMPSession {
  ...
  stop() {
    if (this.isStarting) {
      ...
      Logger.log(`${colors.magenta('[rtmp]')} ${this.ip} disconnected (${colors.cyan('ID:')} ${this.id})`);
  ...
  reject() {
    Logger.log(`${colors.magenta('[rtmp]')} ${this.ip} rejected (${colors.cyan('ID:')} ${this.id})`);
    this.stop();
  }
  ...
  rtmpAudioHandler() {
    ...
    if (this.audioCodec == 0) {
      ...
      if (sound_format != 10) {
        Logger.log(
          `${colors.magenta('[rtmp-publish]')} ${this.ip} sent audio, parsing: (${colors.cyan('streamPath')}: ${colors.yellow(this.publishStreamPath)}, ` +
          `${colors.cyan('codec')}: ${this.audioCodecName}, ${colors.cyan('sampleRate')}: ${this.audioSamplerate}, ${colors.cyan('channels')}: ${this.audioChannels}ch)`
        );
      }
    }
    ...
    let flvTag = Flv.createFlvTag(packet);
    ...
    for (let playerId of this.players) {
      ...
      } else if (playerSession instanceof Flv) {
  ...
  rtmpVideoHandler() {
    ...
    if (this.videoCodec == 0) {
      ...
      Logger.log(
        `${colors.magenta('[rtmp-publish]')} ${this.ip} sent video, parsing: (${colors.cyan('streamPath')}: ${colors.yellow(this.publishStreamPath)}, ` +
        `${colors.cyan('codecName')}: ${this.videoCodecName}, ${colors.cyan('resolution')}: ${this.videoWidth}x${this.videoHeight})`
      );
    }
  ...
  onConnect(invokeMessage) {
    ...
    Logger.log(`${colors.magenta('[rtmp]')} ${this.ip} connected (${colors.cyan('ID:')} ${this.id}, ${colors.cyan('flashVer:')} ${invokeMessage.cmdObj.flashVer})`);
  ...
  onPublish(invokeMessage) {
    ...
    if (context.publishers.has(this.publishStreamPath)) {
      Logger.log(`${colors.magenta('[rtmp-publish]')} ${this.ip} tried to stream to ${colors.yellow(this.publishStreamPath)} but it already exists (${colors.cyan('ID:')} ${this.id})`);
      this.sendStatusMessage(this.publishStreamId, "error", "NetStream.Publish.BadName", "Stream already publishing");
    } else if (this.isPublishing) {
      Logger.log(`${colors.magenta('[rtmp-publish]')} ${this.ip} NetConnection is publishing to ${colors.yellow(this.publishStreamPath)} (${colors.cyan('ID:')} ${this.id})`);
      this.sendStatusMessage(this.publishStreamId, "error", "NetStream.Publish.BadConnection", "Connection already publishing");
    } else {
      Logger.log(`${colors.magenta('[rtmp-publish]')} ${this.ip} published stream to ${colors.yellow(this.publishStreamPath)} (${colors.cyan('ID:')} ${this.id})`);
  ...
  onPlay(invokeMessage) {
    ...
    if (this.isPlaying) {
      Logger.log(`${colors.magenta('[rtmp-play]')} ${this.ip} NetConnection is playing from ${colors.yellow(this.playStreamPath)} (${colors.cyan('ID:')} ${this.id})`);
    ...
    if (context.publishers.has(this.playStreamPath)) {
      ...
      Logger.log(`${colors.magenta('[rtmp-play]')} ${this.ip} requested stream on path ${colors.yellow(this.playStreamPath)} but it was not found (${colors.cyan('ID:')} ${this.id})`);
      this.isIdling = true;
  ...
  onStartPlay() {
    ...
    Logger.log(`${colors.magenta('[rtmp-play]')} ${this.ip} joined stream on path ${colors.yellow(this.playStreamPath)} (${colors.cyan('ID:')} ${this.id})`);
  }

  onPause(invokeMessage) {
    this.isPause = invokeMessage.pause;
    let c = this.isPause ? "NetStream.Pause.Notify" : "NetStream.Unpause.Notify";
    let d = this.isPause ? "paused" : "unpaused";
    Logger.log(`${colors.magenta('[rtmp-play]')} ${this.ip} ${d} the live stream on ${colors.yellow(this.playStreamPath)} (${colors.cyan('ID:')} ${this.id})`);
    if (!this.isPause) {
      this.sendStreamStatus(STREAM_BEGIN, this.playStreamId);
      if (context.publishers.has(this.playStreamPath)) {
        //fix ckplayer
  ...
  onReceiveAudio(invokeMessage) {
    ...
    Logger.log(`${colors.magenta('[rtmp-publish]')} ${this.ip} receiveAudio: ${this.isReceiveAudio} (${colors.cyan('ID:')} ${this.id})`);
  }

  onReceiveVideo(invokeMessage) {
    this.isReceiveVideo = invokeMessage.bool;
    Logger.log(`${colors.magenta('[rtmp-publish]')} ${this.ip} receiveVideo: ${this.isReceiveVideo} (${colors.cyan('ID:')} ${this.id})`);
  }
  ...
  onDeleteStream(invokeMessage) {
    if (invokeMessage.streamId == this.playStreamId) {
      ...
      Logger.log(`${colors.magenta('[rtmp-play]')} ${this.ip} closed stream on path ${colors.yellow(this.playStreamPath)} (${colors.cyan('ID:')} ${this.id})`);
      if (this.isStarting) {
    ...
    if (invokeMessage.streamId == this.publishStreamId) {
      if (this.isPublishing) {
        Logger.log(`${colors.magenta('[rtmp-publish]')} ${this.ip} stopped streaming to path ${colors.yellow(this.publishStreamPath)} (${colors.cyan('ID:')} ${this.id})`);
